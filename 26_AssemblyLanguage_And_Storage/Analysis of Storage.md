...
1, 首先执行`readelf -W -a TestStorage.out`, 展示ELF文件信息，可以看到变量A如下所示。显示的地址都是16进制。int A 占4个byte，地址是0x400600，这是它的起始地址，也代表其内存地址。
...
可以看到A位于`.rodata`行。因为`.rodata`地址范围是：0x5f0 + 0x19 = 0x60f，所以0x600在这个范围内。
size = 0x20, 所以上面 +0x19。
...
# 此处可知 A和Hello C共占0x20(32)个bytes
