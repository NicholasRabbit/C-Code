...
2.  `%esp`寄存器总是指向栈顶，在x86平台上这个栈是从高地址向低地址增长的，每次压栈push的时候esp的地址就减4，弹栈pop就加4；
   `%ebp`总是指向栈底，即`%ebp`的值一直是保存的编号是栈底的，如果使用栈其它位置的数据时用例如`-0x4(%ebp)`等相对位置的数来表示；
...
4. 每次调用一个函数操作系统都要分配一个栈帧来保存参数和局部变量。栈帧和寄存器不一样。
   **栈帧(stack frame) 是什么？**
   栈帧是过程活动记录，存储出栈入栈的顺序，栈帧就是利用`%ebp`(注意：不是`%esp`)来执行相关访问物理内存地址，进行下一步操作的。 
...
一，使用```objdump  -dS  Test.out``` 反汇编把C和Assembly同时展示出来。 个人得到的是%rsp等，寄存器以r开头，教程中是%esp，具体原因是rsp是x64平台的，esp是x86。本人打印结果如下。

[个人反编译结果](./note-images/objdump result.txt)

二， 从 main函数开始分析，上面是本人打印的，下面是老师打印的，可对比分析。

1. 调用函数foo(2, 3)时，3先保存进%esp, 然后2在保存到0x4%esp，由此可知函数是由左向右一次压栈的。
   [老师反编译结果(在源目录打开，这里打开内容窜行)](./note-images/teacher's objdump.txt)
