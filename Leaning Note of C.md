...
2，  在`gdb`中表示寄存器名时前面要加个`$`，例如`p $esp`可以打印`esp`寄存器的值 

**分析：**

1, 执行```gdb x/20 $esp```，打印从地址0xbff1c3f4开始的20个32位数。
...
1，打印的数是16进制，每个数有八位(=32bit)，每两位是一个byte，因此一个数代表4个byte，每行16个byte；
2，十位数上一个代表16，因此有16个地址，正好对应每行的16个byte。



2，这里%rsp等寄存器以r开头相当于%esp，具体原因待分析。

<img src="note-images/1703475550477.png" alt="1703475550477" style="zoom:67%;" />